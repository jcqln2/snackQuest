...
func draw_store_interior():
	...
	# Calorie Mate display
	var item_x = cam_pos.x - 100
	var item_y = cam_pos.y
	
	# Calorie Mate box (yellow/gold)
	draw_rect(Rect2(item_x, item_y, 60, 40), Color(0.9, 0.7, 0.2))
	draw_rect(Rect2(item_x + 5, item_y + 5, 50, 30), Color(1.0, 0.85, 0.4))
	
	# Price tag
	draw_rect(Rect2(item_x + 10, item_y + 45, 40, 15), Color(1, 1, 1))
	
	# UI Panel
	var ui_x = cam_pos.x - 300
	var ui_y = cam_pos.y - 200
	
	draw_rect(Rect2(ui_x, ui_y, 600, 300), Color(0.2, 0.2, 0.2, 0.9))
	draw_rect(Rect2(ui_x + 5, ui_y + 5, 590, 290), Color(0.3, 0.3, 0.3))
	
	# Title
	draw_rect(Rect2(ui_x + 20, ui_y + 20, 200, 30), Color(1, 1, 1))
	
	# Instructions
	var text_y = ui_y + 80
	draw_rect(Rect2(ui_x + 20, text_y, 560, 25), Color(0.9, 0.9, 0.9))
	...
	text_y += 50
	# Player stats
	draw_rect(Rect2(ui_x + 20, text_y, 560, 25), Color(0.7, 1.0, 0.7))
	
	text_y += 35
	draw_rect(Rect2(ui_x + 20, text_y, 560, 25), Color(1.0, 1.0, 0.7))
...
#var gravity = 980
#var is_on_ground = false
#var ground_level = 970  # Ground/sidewalk level (670 + 300)
#var stars = []  # Array to hold star data
#var time_passed = 0.0
#var rats = []  # Array to hold rat enemies
#var rat_spawn_timer = 0.0
...
#func _draw():
	...
	## Draw "GAME OVER" if dead
	#if !is_alive:
		#var game_over_pos = Vector2(player_pos.x - 100, player_pos.y - 150)
		## Simple text using rectangles (GAME OVER)
		#draw_rect(Rect2(game_over_pos.x, game_over_pos.y, 200, 40), Color(1, 0, 0, 0.8))
#
#func draw_health_bar():
	## Draw health bar in top-left of screen (relative to camera)
	#var cam_pos = $Camera2D.position
